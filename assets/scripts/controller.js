...
const controller = function (app) {
    ...
    firebase.auth.onAuthStateChanged(user => {
        if (user != null) {
            ...
            app.post('/get-user-info', urlencodedParser,(req, res) => {               
                // console.log('id: ', req.body.id)                
                firebase.firestore.collection('user data').where('accountNo', '==', req.body.id).get().then(snapshot=>{
                    res.send(snapshot.docs[0].data().info);
                })
            });
                    ...
                    snapshot.docs.forEach(doc => {
                        ...
                        if (req.body.keystroke.length === 0) {
                            ...
                            if (result) {//matches the name given
                                ...
                                matches.push({ name: doc.data().name, id: doc.id, email: doc.data().email, info: doc.data().info, picture: doc.data().profilePicture });
                            }
                        }
